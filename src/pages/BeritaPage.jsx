...
export default function BeritaPage() {
  ...
  // PERMISSION: Apakah boleh hapus berita? (Guru/Admin)
  const canDelete = canAccess(user, 'POST_NEWS');
  ...
  const handleDelete = async (e, id) => {
      ...
      if(confirm("Yakin mau hapus berita ini permanen?")) {
          ...
          } catch (error) { triggerToast("Gagal: " + error.message, "error"); }
      }
  }
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="bg-[#002f6c] text-white py-10 px-4 shadow-md">
            <div className="max-w-6xl mx-auto flex justify-between items-start">
                ...
                {canDelete && <div className="bg-white/10 px-4 py-2 rounded-lg text-xs font-bold text-green-300 flex items-center gap-2"><CheckCircle size={14}/> Mode Editor</div>}
            </div>
        </div>

        <div className="max-w-6xl mx-auto px-4 py-10">
            {loading ? <div className="text-center">Loading...</div> : newsList.length === 0 ? <div className="text-center py-20 text-slate-400">Belum ada berita.</div> : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {newsList.map((item) => (
                        <div key={item.id} onClick={() => setSelectedNews(item)} className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all border border-slate-100 flex flex-col h-full cursor-pointer relative">
                            {canDelete && (
                                <button onClick={(e) => handleDelete(e, item.id)} className="absolute top-2 right-2 z-20 bg-red-600 text-white p-2 rounded-full shadow-lg hover:bg-red-700 hover:scale-110 transition-all"><Trash2 size={16} /></button>
                            )}
                            <div className="h-48 overflow-hidden relative bg-slate-100">
                                {item.image ? <img src={item.image} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/> : <div className="w-full h-full flex items-center justify-center text-slate-300"><Tag size={40}/></div>}
                ...
                <div className="bg-white w-full md:max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                    <div className="relative h-64 shrink-0 bg-slate-100">
                        ...
                        <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 to-transparent p-6 pt-20">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-white/80 text-xs font-medium"><span className="flex items-center gap-1"><Calendar size={12}/> {selectedNews.dateString}</span><span>Oleh: {selectedNews.author}</span></div>
                        </div>
                    </div>
