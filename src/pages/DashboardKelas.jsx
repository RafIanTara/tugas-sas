...
function DashboardKelas({ kelasId }) { 
    ...
    const handleSaveSettings = async (e) => { e.preventDefault(); try { if (apiKeyInput.trim()) await setDoc(doc(db, 'settings', 'ai_config'), { apiKey: apiKeyInput, model: selectedModel, updatedAt: serverTimestamp() }); await setDoc(doc(db, 'settings', `${dbPrefix}kas_config`), { saldoAwal: parseInt(saldoAwal), nominal: parseInt(nominalKas), updatedAt: serverTimestamp() }); triggerToast("Konfigurasi Disimpan!"); setIsSettingsModalOpen(false); setApiKeyInput(''); } catch (e) { triggerToast("Gagal simpan", "error"); } }
...
Jawab yang asik, solutif, tapi tetap aman buat sekolah ya Wak!
    ...
    return (
        <div className="min-h-screen font-sans bg-slate-100 dark:bg-[#0b1121] text-slate-800 dark:text-slate-100 pb-20 md:pb-0 transition-colors duration-300">
            ...
            <Modal isOpen={isSettingsModalOpen} onClose={() => setIsSettingsModalOpen(false)} title="Konfigurasi"><form onSubmit={handleSaveSettings} className="space-y-4"><div className="flex bg-slate-100 dark:bg-slate-700 p-1 rounded-lg mb-3"><button type="button" onClick={() => setSettingTab('ai')} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${settingTab === 'ai' ? 'bg-white text-[#002f6c] shadow-sm' : 'text-slate-500'}`}>AI & API</button><button type="button" onClick={() => setSettingTab('kas')} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${settingTab === 'kas' ? 'bg-white text-[#002f6c] shadow-sm' : 'text-slate-500'}`}>Data Kelas</button></div>{settingTab === 'ai' && (<div className="space-y-3 animate-in fade-in"><div><label className="block text-xs font-bold text-slate-500 mb-1">Gemini API Key</label><input type="text" value={apiKeyInput} onChange={e => setApiKeyInput(e.target.value)} className="w-full bg-white border border-slate-300 rounded-lg p-2 text-slate-800 text-xs outline-none focus:border-blue-500" placeholder="Paste Key..." /></div><div><label className="block text-xs font-bold text-slate-500 mb-1">Model</label><select value={selectedModel} onChange={e => setSelectedModel(e.target.value)} className="w-full bg-white border border-slate-300 rounded-lg p-2 text-slate-800 text-xs outline-none"><option value="gemini-1.5-flash">1.5 Flash</option><option value="gemini-2.5-flash">2.5 Flash</option></select></div></div>)}{settingTab === 'kas' && (<div className="space-y-3 animate-in fade-in"><div><label className="block text-xs font-bold text-slate-500 mb-1">Saldo Awal (Rp)</label><input type="number" value={saldoAwal} onChange={e => setSaldoAwal(e.target.value)} className="w-full bg-white border border-slate-300 rounded-lg p-2 text-slate-800 text-xs outline-none" /></div><div className="border-t border-slate-200 pt-3"><label className="block text-xs font-bold text-slate-500 mb-2">Siswa</label><div className="flex gap-2 mb-2"><input type="text" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} className="flex-1 bg-white border border-slate-300 rounded-lg p-2 text-xs outline-none" placeholder="Nama..." /><button type="button" onClick={handleAddStudent} className="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700"><Plus size={14} /></button></div><div className="max-h-[100px] overflow-y-auto custom-scrollbar space-y-1 bg-slate-50 p-2 rounded-lg border border-slate-200">{loadingStudents ? <p className="text-xs text-slate-400">Loading...</p> : dataStudents.sort((a, b) => a.name.localeCompare(b.name)).map(s => (<div key={s.id} className="flex justify-between items-center p-1.5 bg-white rounded border border-slate-100"><span className="text-xs font-medium text-slate-700">{s.name}</span><button type="button" onClick={() => handleDeleteStudent(s.id)} className="text-red-400 hover:text-red-600"><X size={12} /></button></div>))}</div></div></div>)}<button className="w-full bg-[#00994d] text-white py-2.5 rounded-lg text-sm font-bold shadow-md hover:bg-green-700 mt-2">Simpan</button></form></Modal>
