...
function App() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'model', text: 'Halo Wak! Gue asisten AI TKJ. Ada yang bisa dibantu? 😎' }
  ])
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
        try {
            ...
            if (docSnap.exists() && docSnap.data().model) setSelectedModel(docSnap.data().model);
  ...
  const handleSaveSettings = async (e) => {
    e.preventDefault(); if (!apiKeyInput.trim()) { alert("API Key kosong!"); return; }
    try { await setDoc(doc(db, 'settings', 'ai_config'), { apiKey: apiKeyInput, model: selectedModel, updatedAt: serverTimestamp() }); alert(`Setting tersimpan! Model: ${selectedModel}`); setIsSettingsModalOpen(false); setApiKeyInput('') } catch (error) { alert("Gagal simpan setting.") }
  }
  ...
  const handleSendChat = async (e) => {
    ...
    try {
        ...
        let dbApiKey = "";
        let dbModel = "gemini-1.5-flash"; 

        if (docSnap.exists()) { 
            const data = docSnap.data();
            dbApiKey = data.apiKey;
            if(data.model) dbModel = data.model;
        } else { 
            throw new Error("Admin belum setting API Key!"); 
        }

        const genAI = new GoogleGenerativeAI(dbApiKey);
        const model = genAI.getGenerativeModel({ model: dbModel });

        // Context Memory (6 Chat Terakhir)
        const conversationContext = chatHistory.slice(-6).map(msg => `${msg.role === 'user' ? 'User' : 'AI'}: ${msg.text}`).join('\n');
        
        const finalPrompt = `
          Kamu Asisten AI TKJ. Gaya santai.
          RIWAYAT CHAT:
          ${conversationContext}
          PERTANYAAN: "${currentText}"
        `;

        let result;
        if (currentImageFile) {
            const imagePart = await fileToGenerativePart(currentImageFile);
            result = await model.generateContent([currentText || "Jelaskan gambar ini", imagePart]);
        } else {
        ...
        if (error.message.includes("403")) msg = "Akses Ditolak (403). Cek API Key.";
        if (error.message.includes("404")) msg = "Model Tidak Ditemukan.";
  ...
  return (
    <div className="min-h-screen text-white font-sans pb-20 md:pb-0">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div className="flex-1 w-full">
            <div className="flex justify-between items-center mb-1">
                ...
                {/* Setting button - visible on mobile header too */}
                {isAdmin && (
            ...
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-slate-400 tracking-tighter drop-shadow-sm">XI <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">TKJ</span></h1>
                    <p className="text-slate-400 text-xs md:text-sm mt-1 flex items-center gap-2"><span className="relative flex h-2 w-2"><span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isOnline ? 'bg-green-400' : 'bg-red-400'}`}></span><span className={`relative inline-flex rounded-full h-2 w-2 ${isOnline ? 'bg-green-500' : 'bg-red-500'}`}></span></span> <span className={`font-bold ${isOnline ? 'text-green-400' : 'text-red-400'}`}>{isOnline ? 'Online' : 'Offline'}</span></p>
                </div>
                
                {/* Jam Digital (Hidden di Mobile biar ringkas) */}
                <div className="hidden md:block bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 p-4 rounded-2xl text-right">
                    <div className="text-3xl font-bold font-mono text-white">{time.toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'})}</div>
                    <div className="text-sm text-slate-400">{time.toLocaleDateString('id-ID', {weekday: 'long', day: 'numeric', month: 'long'})}</div>
                </div>
            </div>
          </div>
        </header>
      ...
      {/* CONTENT GRID - LAYOUT BEDA UNTUK MOBILE */}
      <div className="p-4 md:p-8 pt-4 max-w-7xl mx-auto flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-6">
        
        {/* 1. INFO PENTING (MOBILE: TOP, DESKTOP: RIGHT) */}
        <div className="order-1 md:col-span-4 md:order-3">
            <div className="h-full bg-gradient-to-br from-indigo-900/80 to-slate-900/80 backdrop-blur-md p-5 rounded-3xl border border-indigo-500/30 relative overflow-hidden group shadow-lg">
                <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/20 blur-[60px] rounded-full animate-pulse"></div>
                <div className="flex items-center justify-between mb-2 relative z-10">
                    <div className="flex items-center gap-2 bg-indigo-500/20 px-3 py-1 rounded-full border border-indigo-500/30"><Megaphone size={14} className="text-indigo-300" /> <span className="text-xs font-bold text-indigo-100 uppercase">Broadcast</span></div>
                    {isAdmin && <button onClick={openInfoModal} className="opacity-60 hover:opacity-100 hover:text-white transition-opacity"><Edit3 size={16} /></button>}
                </div>
                <div className="relative z-10">{dataInfo.length > 0 ? (<p className="text-indigo-50 text-sm font-medium leading-relaxed whitespace-pre-wrap line-clamp-3 hover:line-clamp-none transition-all cursor-pointer">{dataInfo.find(i => i.id === 'info_utama')?.isi || dataInfo[0]?.isi}</p>) : <div className="text-white/40 italic text-sm">Info kosong.</div>}</div>
            </div>
        </div>

        {/* 2. QUICK LINKS (MOBILE: SCROLLABLE ROW) */}
        <div className="order-2 md:col-span-4 md:order-2">
           <div className="bg-slate-900/60 backdrop-blur-md p-4 md:p-5 rounded-3xl border border-slate-700/50 flex md:justify-around items-center gap-4 overflow-x-auto scrollbar-hide">
              {/* AI CHAT */}
              <div onClick={() => setIsAiModalOpen(true)} className="flex flex-col items-center gap-2 cursor-pointer group min-w-[60px]">
                 <div className="p-3 rounded-2xl bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 shadow-[0_0_10px_rgba(250,204,21,0.2)]"><Zap size={20} fill="currentColor" /></div>
                 <span className="text-[10px] font-medium text-slate-400">AI Chat</span>
              </div>
              {/* CREATOR */}
              <div onClick={() => window.open('https://rafiantara.fun', '_blank')} className="flex flex-col items-center gap-2 cursor-pointer group min-w-[60px]">
                 <div className="p-3 rounded-2xl bg-green-400/10 text-green-400 border border-white/5"><Globe size={20} /></div>
                 <span className="text-[10px] font-medium text-slate-400">Creator</span>
              </div>
              {/* TOOLS LAIN */}
              ...
              <div className="flex flex-col items-center gap-2 cursor-pointer group opacity-50 min-w-[60px]"><div className="p-3 rounded-2xl bg-purple-400/10 text-purple-400 border border-white/5"><Calculator size={20} /></div><span className="text-[10px] font-medium text-slate-400">Calc</span></div>
           </div>
        </div>

        {/* 3. PIKET & JADWAL (STACKED ON MOBILE) */}
        
        {/* Piket */}
        <div className="order-3 md:col-span-4 md:order-1">
            <div className="bg-gradient-to-br from-orange-900/40 to-slate-900 p-5 rounded-3xl border border-orange-500/20 relative overflow-hidden group shadow-lg h-full">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center border border-orange-600/30"><UserCheck size={20} className="text-orange-400" /></div><div><h3 className="font-bold text-white text-sm">Piket</h3><p className="text-[10px] text-slate-400">{hariPilihan}</p></div></div>
                    {isAdmin && !isLibur && (<button onClick={openPiketModal} className="text-orange-400 hover:text-orange-300 bg-slate-800 p-2 rounded-full"><Edit3 size={14} /></button>)}
                </div>
                {/* Tombol Hari di Piket biar Navigasi gampang */}
                <div className="flex gap-2 overflow-x-auto pb-2 mb-2 scrollbar-hide md:hidden">
                    {['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'].map((hari) => (<button key={hari} onClick={() => setHariPilihan(hari)} className={`px-2 py-1 rounded-md text-[10px] font-bold border ${hariPilihan === hari ? 'bg-orange-600 border-orange-500' : 'bg-transparent border-slate-700 text-slate-400'}`}>{hari.substring(0, 3)}</button>))}
                </div>
                <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 text-center">
                    {isLibur ? <span className="text-xs text-orange-200">Libur Cuy! ☕</span> : piketTampil ? (<div className="flex flex-wrap gap-2 justify-center">{piketTampil.names.map((nama, idx) => (<span key={idx} className="bg-orange-500/20 text-orange-200 text-xs px-2 py-1 rounded-md border border-orange-500/30">{nama}</span>))}</div>) : <span className="text-xs text-slate-500 italic">Kosong.</span>}
                </div>
            </div>
        </div>

        {/* Jadwal Mapel */}
        <div className="order-4 md:col-span-8 md:order-4">
            <div className="bg-slate-900/60 backdrop-blur-md p-5 rounded-3xl border border-slate-700/50 shadow-xl h-full flex flex-col">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                    <div className="flex items-center gap-3"><div className="p-2 bg-blue-500/10 rounded-lg"><BookOpen size={20} className="text-blue-400" /></div><div><h2 className="font-bold text-lg text-white">Jadwal</h2><p className="text-[10px] text-slate-500 uppercase tracking-wider">{hariPilihan}</p></div></div>
                    {/* Tombol Hari Desktop */}
                    <div className="w-full md:w-auto flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                        {['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'].map((hari) => (<button key={hari} onClick={() => setHariPilihan(hari)} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border whitespace-nowrap ${hariPilihan === hari ? 'bg-blue-600 border-blue-500 text-white' : 'bg-transparent border-slate-700 text-slate-400'}`}>{hari}</button>))}
                        {isAdmin && !isLibur && <button onClick={openScheduleModal} className="px-3 py-1.5 bg-slate-800 rounded-lg border border-slate-700 text-slate-400"><Edit3 size={14} /></button>}
                    </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {isLibur ? <div className="col-span-full py-8 text-center text-slate-500 border-2 border-dashed border-slate-800 rounded-xl"><Coffee size={32} className="mx-auto mb-2 opacity-50"/><p className="text-xs">Libur, Rehat dulu.</p></div> : loadingJadwal ? <p className="text-slate-500 text-xs">Loading...</p> : jadwalTampil ? (jadwalTampil.mapel.map((mapel, index) => (<div key={index} className="bg-slate-800/40 border border-slate-700/50 p-3 rounded-lg flex items-center gap-2"><span className="w-6 h-6 rounded bg-slate-900 flex items-center justify-center text-[10px] font-bold text-slate-500">{index + 1}</span><span className="font-medium text-slate-300 text-sm truncate">{mapel}</span></div>))) : <div className="col-span-full py-4 text-center text-slate-500 text-sm border-2 border-dashed border-slate-800 rounded-xl">Jadwal Kosong.</div>}
                </div>
            </div>
        </div>
        
        {/* 5. QUOTE (FILLER) */}
        <div className="order-5 md:col-span-4">
           <div className="h-full bg-slate-900/60 backdrop-blur-md p-4 rounded-3xl border border-slate-700/50 flex flex-col justify-center text-center relative overflow-hidden min-h-[100px]">
              {isAdmin && (<button onClick={() => setIsQuoteModalOpen(true)} className="absolute top-3 right-3 text-slate-500 hover:text-blue-400 p-1 bg-slate-800/50 rounded-full z-10"><Edit3 size={12} /></button>)}
              <AnimatePresence mode="wait">
                <motion.div key={quote.text} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="flex flex-col items-center px-2">
                  <Quote size={20} className="text-blue-500/30 mb-1"/>
                  <p className="text-slate-300 italic font-serif text-sm leading-relaxed">"{quote.text}"</p>
                  <p className="text-slate-500 text-[10px] mt-2 font-bold uppercase tracking-widest">— {quote.author}</p>
                </motion.div>
              </AnimatePresence>
           </div>
        </div>
        
        {/* 6. TUGAS (BOTTOM LIST - MOBILE FRIENDLY) */}
        <div className="order-6 md:col-span-12">
            <div className="bg-slate-900/60 backdrop-blur-md p-5 rounded-3xl border border-slate-700/50 shadow-xl min-h-[300px]">
              <div className="flex items-center justify-between mb-4">
                 <div className="flex items-center gap-3"><div className="bg-green-500/10 p-2 rounded-lg text-green-400"><CheckSquare size={20} /></div><h2 className="font-bold text-lg">Tugas <span className="text-slate-500 text-sm font-normal">({daftarTugas.filter(t => !t.selesai).length})</span></h2></div>
                 {isAdmin && <button onClick={() => setIsTaskModalOpen(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded-xl font-bold text-xs shadow-lg flex items-center gap-1"><Plus size={14} /> Baru</button>}
              </div>
              {/* Scrollable di Desktop, Memanjang di Mobile */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                 {!loadingTugas && daftarTugas.length === 0 && <div className="col-span-full text-center text-slate-500 py-8 text-sm">Tugas Bersih! ✨</div>}
                 {daftarTugas.map((tugas) => (
                   <div key={tugas.id} onClick={() => isAdmin && handleToggleTask(tugas.id, tugas.selesai)} className={`relative p-4 rounded-2xl border transition-all cursor-pointer group hover:-translate-y-1 hover:shadow-lg ${tugas.selesai ? 'bg-slate-900/30 border-slate-800 opacity-50' : 'bg-slate-800/40 border-slate-700 hover:border-blue-500/50'} ${!isAdmin ? 'cursor-default hover:translate-y-0' : ''}`}>
                     <div className="flex justify-between items-start">
                        <div className="flex-1 min-w-0 pr-2">
                          <h3 className={`font-semibold text-sm mb-1 truncate ${tugas.selesai ? 'line-through text-slate-500' : 'text-white'}`}>{tugas.judul}</h3>
                     ...
                     {isAdmin && <button onClick={(e) => handleDeleteTask(e, tugas.id)} className="absolute bottom-3 right-3 text-slate-500 hover:text-red-400 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900/50 p-1 rounded-full"><Trash2 size={14} /></button>}
                   </div>
                 ))}
              </div>
           </div>
