...
import { BookOpen, CheckSquare, Plus, Trash2, X, Edit3, Megaphone, Calendar, Link, Wifi, Calculator, Zap, Send, Bot, Globe, Lock, Unlock, LogOut, Image as ImageIcon, XCircle, UserCheck, Coffee, Settings, Key } from 'lucide-react'
...
function App() {
  ...
  const [isSettingsModalOpen, setIsSettingsModalOpen] = useState(false) // Modal Setting Key
  const [pinInput, setPinInput] = useState('')
  const [apiKeyInput, setApiKeyInput] = useState('') // Input buat Key Baru
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'model', text: 'Halo Wak! Gue asisten AI TKJ. Kunci otak gue sekarang diambil dari Database biar aman. 😎' }
  ])
  ...
  // NEW: HANDLE SIMPAN API KEY KE DATABASE
  const handleSaveApiKey = async (e) => {
    e.preventDefault()
    if (!apiKeyInput.trim()) return
    try {
        // Simpan ke koleksi 'settings', dokumen 'ai_config'
        await setDoc(doc(db, 'settings', 'ai_config'), { 
            apiKey: apiKeyInput,
            updatedAt: serverTimestamp() 
        })
        alert("API Key berhasil disimpan di Database! 🧠")
        setIsSettingsModalOpen(false)
        setApiKeyInput('')
    } catch (error) {
        console.error("Gagal simpan key:", error)
        alert("Gagal simpan key. Cek koneksi.")
    }
  }
  ...
  const handleSendChat = async (e) => {
    ...
    try {
        // 1. AMBIL API KEY DARI DATABASE (BUKAN .ENV LAGI)
        const docRef = doc(db, "settings", "ai_config");
        const docSnap = await getDoc(docRef);
        
        let dbApiKey = "";
        if (docSnap.exists()) {
            dbApiKey = docSnap.data().apiKey;
        } else {
            throw new Error("API Key belum disetting di Database! (Masuk Admin -> Settings)");
        }

        // 2. INISIALISASI GEMINI
        const genAI = new GoogleGenerativeAI(dbApiKey);
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" }); // Pakai 1.5 Flash Stabil
        ...
        if (currentImageFile) {
            ...
            const prompt = `Kamu Asisten AI TKJ. Jawab santai: "${currentText}"`;
        ...
        if (error.message.includes("403") || error.message.includes("404")) msg = "API Key di Database SALAH/EXPIRED. Ganti lewat menu Admin!";
  ...
  return (
    <div className="min-h-screen text-white font-sans pb-10 md:pb-0">
      ...
      <div className="p-4 md:p-8 pt-6 max-w-7xl mx-auto flex flex-col md:grid md:grid-cols-12 gap-5 md:gap-6">
        {/* JADWAL PIKET */}
        <div className="md:col-span-4 order-3 md:order-1"><div className="bg-gradient-to-br from-orange-900/40 to-slate-900 p-5 rounded-3xl border border-orange-500/20 relative overflow-hidden group shadow-lg flex flex-col h-full"><div className="flex items-center justify-between mb-4"><div className="flex items-center gap-3"><div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center border border-orange-600/30"><UserCheck size={24} className="text-orange-400" /></div><div><h3 className="font-bold text-white">Jadwal Piket</h3><p className="text-xs text-slate-400">{hariPilihan}</p></div></div>{isAdmin && !isLibur && (<button onClick={openPiketModal} className="text-orange-400 hover:text-orange-300 bg-slate-800 p-2 rounded-full transition-colors"><Edit3 size={16} /></button>)}</div><div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-hide">{['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'].map((hari) => (<button key={hari} onClick={() => setHariPilihan(hari)} className={`px-2 py-1 rounded-lg text-[10px] font-bold transition-all border whitespace-nowrap ${hariPilihan === hari ? 'bg-orange-600 border-orange-500 text-white' : 'bg-transparent border-slate-700 text-slate-400'}`}>{hari.substring(0, 3)}</button>))}</div><div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 flex-1 flex items-center justify-center">{isLibur ? (<div className="text-center"><Coffee size={32} className="text-orange-400 mx-auto mb-2 opacity-50"/><p className="text-sm font-medium text-orange-200">Libur Cuy! 😎<br/><span className="text-xs text-slate-500">Rebahan dulu...</span></p></div>) : loadingPiket ? <p className="text-xs text-slate-500">Loading...</p> : piketTampil ? (<div className="flex flex-wrap gap-2 justify-center">{piketTampil.names.map((nama, idx) => (<span key={idx} className="bg-orange-500/20 text-orange-200 text-sm px-3 py-1 rounded-lg border border-orange-500/30 font-medium">{nama}</span>))}</div>) : (<div className="text-center"><p className="text-sm font-medium text-slate-500 italic">Belum ada data piket.</p>{isAdmin && <button onClick={openPiketModal} className="text-xs text-orange-400 underline mt-1">Isi sekarang</button>}</div>)}</div></div></div>
      ...
      <Modal isOpen={isSettingsModalOpen} onClose={() => setIsSettingsModalOpen(false)} title="AI Configuration">
        <form onSubmit={handleSaveApiKey} className="space-y-4">
            <div className="bg-yellow-500/10 p-3 rounded-lg border border-yellow-500/20 mb-4">
                <p className="text-xs text-yellow-400 flex items-center gap-2"><Key size={14}/> <b>PENTING:</b> Masukkan API Key Gemini baru di sini jika Chatbot error/limit.</p>
            </div>
            <div>
                <label className="block text-xs text-slate-400 mb-1">Gemini API Key</label>
                <input type="text" value={apiKeyInput} onChange={e => setApiKeyInput(e.target.value)} className="w-full bg-slate-800 border-slate-700 rounded-xl p-3 text-white focus:border-green-500 focus:outline-none" placeholder="Paste key disini..." />
            </div>
            <button className="w-full bg-green-600 py-3 rounded-xl font-bold text-white hover:bg-green-500">Simpan Konfigurasi</button>
        </form>
      </Modal>
